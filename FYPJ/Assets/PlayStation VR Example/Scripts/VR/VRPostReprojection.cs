...
public class VRPostReprojection : MonoBehaviour
{
    ...
    void OnPostRender()
    {
        ...
        if (UnityEngine.XR.XRSettings.loadedDeviceName == UnityEngine.XR.XRSettings.supportedDevices[0])
        {
            ...
            {
                if (currentEye == 0)
                    postReprojectionTexture = PlayStationVR.GetCurrentFramePostReprojectionEyeTexture(UnityEngine.XR.XRNode.LeftEye);
                else if (currentEye == 1)
                    postReprojectionTexture = PlayStationVR.GetCurrentFramePostReprojectionEyeTexture(UnityEngine.XR.XRNode.RightEye);
